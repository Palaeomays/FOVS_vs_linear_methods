function [treturn,calreturn,extrapreturn,newfnhist]=MicrofossilSim_iCheck(Mx,Mn,tlim,fovsx,fovsn,printflag,fnnobins)
    ...
    %treturn of form [transect x count,std dev of x count,right edge of transect;transect n count, std dev of n count,0 (to make array rectangular)]
    treturn=[tnumberx,tsdx,tright;tnumbern,tsdn,0];
    ...
    xfpc=(Mx-fnumberx)/Mx;%finite population correction from Schaeffer (4.2), p.84
    fvarx=(var(fxcounts)/(fmeanx^2))/xfpc;%Proportional sample variance of mean number of fossils per FOV in calibration, corrected with finite population correction
    calc4=sqrt(2/(fx-1))*gamma(fx/2)/gamma((fx-1)/2);%Sampling st dev correction factor for calibration counts

    fovsdx=(std(fxcounts)/fmeanx)/calc4/sqrt(xfpc);%Proportional sample standard deviation, (corrected with c4 and finite population correction from Schaeffer).
    ...
    %%%
    %Testing Poisson/binomial distribution
    newfnhist=histcounts(fncounts,0:fnnobins);
    ...
    nfpc=(Mn-fnumbern)/Mn;%finite population correction (Schaeffer)
    fovsdn=(std(fncounts)/fmeann)/fovc4/sqrt(nfpc);
    ...
    if printflag==1
        ...
        rectangle('Position',[tstartpos(1),tstartpos(2),twidth,theight],'EdgeColor','r','LineWidth',2,'FaceColor','r')
        for i=1:fx
