%Calculate rate of change of precision for change in units of effort
clear;
clc;
close all;
its=100000;
...
%params of form [Mx,Mx,max x to count in linear method transect,fixed number of calibration FOVs, max number of extrapolation count FOVs,omega]
...
%     end
    ...
    [LPrecWorkOpt,FPrecWorkOptBig]=WorkSimV3(Mx,Mn,maxtlim,bigfx,maxfnbig,its,fileflag);
    
    %maxfnmed=min(max(ceil(1.25*optimalfnmed),50),19*19-medfx)
    maxfnmed=19*19-medfx
    fprintf(2,'[fx=%i] batch i=%d, Mx=%d, Mn=%d, maxtlim=%d, maxfn=%d, %s\n',medfx,i,Mx,Mn,maxtlim,maxfnmed,datestr(clock));
    [LPrecWorkOpt,FPrecWorkOptMed]=WorkSimV3(Mx,Mn,maxtlim,medfx,maxfnmed,its,fileflag);
    ...
    [LPrecWorkOpt,FPrecWorkOptSmall]=WorkSimV3(Mx,Mn,maxtlim,smallfx,maxfnsmall,its,fileflag);
    ...
    %FPrecWorkOpt of form: [no. calibration FOVs (fixed); no. extrapolation count FOVs; effort; precision;no. fossils in calibration count (fixed);no. exotics in extrapolation count; concentration estimate; % error in concentration estimate; effort beyond which accuracy goal is met]
