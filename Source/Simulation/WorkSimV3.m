function [LPrecWorkOpt,FPrecWorkOpt] = WorkSimV3(Mx,Mn,tmax,fx,fn,its,fileopt)
...
for i=1:its
    if i==1
        [WorkPrecFAll,WorkPrecLAll]=WorkSimV3_i(Mx,Mn,tmax,calFOVs,extrapFOVs);
    else
        [WorkPrecF,WorkPrecL]=WorkSimV3_i(Mx,Mn,tmax,calFOVs,extrapFOVs);
        ...
        WorkPrecFAll(6,:)=WorkPrecFAll(6,:)+WorkPrecF(6,:);%Extrapolation counts (FOVs)
    end
...
%WorkPrecF of form: [no. calibration FOVs; no. extrapolation count FOVs; effort; precision;no. fossils in calibration count (fixed);no. exotics in extrapolation count]
...
extrapprob=fn*FOVarea/(100*100);
expextrapcount=extrapprob*Mn
actualextrapcount=WorkPrecFAll(6,end)
...
%FPrecWorkOpt of form: [no. calibration FOVs; no. extrapolation count FOVs; effort; precision;no. fossils in cal count (fixed);no. exotics in extrapolation count; concentration estimate; % error in concentration estimate; effort beyond which accuracy goal is met]
