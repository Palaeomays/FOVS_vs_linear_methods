...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} CalculatorFOVSMarker 
   Caption         =   "Absolute abundance calculator v1.1.1 - FOVS method (Marker focus)"
   ClientHeight    =   6285
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   11895
...
Private Sub CommandButton_FocusTargets_Click()
    response = MsgBox("Are you sure you want to change the focus to targets [x]?", vbQuestion + vbYesNo + vbDefaultButton2, "Changing focus")
    ' Check user's response
    If response = vbYes Then
...
'    End If
    ...
    For Each ws In ThisWorkbook.Worksheets
        If ws.Name = "Saved Variables (FOVS_Marker)" Then
    ...
    SavedVariablesFOVSMarker.Cells(nextRow, "E").Value = txt_N3E.Value
    ...
    SavedVariablesFOVSMarker.Cells(nextRow, "AB").Value = CountingEffort.txt_TimeFOV.Value
    SavedVariablesFOVSMarker.Cells(nextRow, "AC").Value = CountingEffort.txt_TimeTotal.Value
    
    ' Inform the user that values have been saved
    InfoExported = True
    MsgBox "Values have been saved to the worksheet 'Saved Variables (FOVS_Marker)'.", vbInformation
...
Private Sub AddHeadersFOVSMarker(ByRef ws As Worksheet)
    With ws
        ...
        .Cells(1, 4).Value = "Number of target specimens from extrapolation counts [x]"
        ...
        .Cells(1, 24).Value = "Estimate of marker specimens from extrapolation counts [nhat]"
...
' Validate other input fields to see if not empty.
   ...
   ' Perform background calculations
    ...
    ' Y3n
        ...
        LabelResult_Y3n.Text = Format(Y3n, "0.000")
    ...
    ' Check if X values exist before running following calculations.
    
    If Len(txt_X_FOVS.Text) = 0 Then
        MsgBox "Please enter the number of targets counted in the extrapolation counts [X].", vbExclamation, "Input Required"
        txt_X_FOVS.SetFocus
        Exit Sub
    End If
...
'    CommandButton_SaveVariables_FOVS.Enabled = True
    ...
    If X > nhat And Not MethodSwitchIgnored Then
        ...
        Else
            ...
            CommandButton_FocusTargets.Enabled = True
        End If
    Else
    End If
End Sub
    ...
    Private Sub UserForm_QueryClose(Cancel As Integer, CloseMode As Integer) ' Once UserForm is closed, close all open windows and clear variables.
        ...
        If CloseMode = 0 Then
            ...
            ElseIf Not SavedVariablesFOVSMarkerExists And OutputsSaved Then
                ...
                End If
            ElseIf (txt_X_FOVS.Value <> "" Or txt_N3E.Value <> "" Or txt_LevelError.Value <> "") Or (SavedMarkerDetails Or CalibratedFOV) Then
                Cancel = 1 ' Cancel the close operation
