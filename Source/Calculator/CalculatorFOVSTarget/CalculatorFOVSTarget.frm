...
Private Sub CommandButton_FocusMarkers_Click()
    response = MsgBox("Are you sure you want to change the focus to markers [n]?", vbQuestion + vbYesNo + vbDefaultButton2, "Clear Inputs")
    ' Check user's response
    If response = vbYes Then
...
Private Sub CommandButton_SaveVariables_FOVS_Click()
    ...
    SavedVariablesFOVSTarget.Cells(nextRow, "E").Value = txt_N3f.Value
    ...
    SavedVariablesFOVSTarget.Cells(nextRow, "AB").Value = CountingEffort.txtTimeFOV.Value
    SavedVariablesFOVSTarget.Cells(nextRow, "AC").Value = CountingEffort.txtTimeTotal.Value
        
    ' Inform the user that values have been saved
    InfoExported = True
    MsgBox "Values have been saved to the worksheet 'Exported data (FOVS-t)'.", vbInformation
...
Private Sub AddHeadersFOVSTarget(ByRef ws As Worksheet)
    With ws
        ...
        .Cells(1, 4).Value = "Number of marker specimens from full counts [n]"
        ...
        .Cells(1, 24).Value = "Estimate of target specimens extrapolated from full counts [xhat]"
...
Private Sub UserForm_Initialize()
    ...
    If CalibratedFOV Then
        txt_N_FOVS.Enabled = True
        txt_N_FOVS.BackColor = RGB(255, 255, 255)
        
        txt_N3f.Enabled = True
        txt_N3f.BackColor = RGB(255, 255, 255)
    Else
        txt_N_FOVS.BackColor = RGB(224, 224, 224)
...
Private Sub CommandButton_Calculate_FOVS_Click()
   ' Validate other input fields to see if not empty.
    ...
    If Not IsNumeric(txt_N_FOVS.Value) Then
        MsgBox "Please enter the number of markers counted in the full counts [n].", vbExclamation, "Input Required"
        Exit Sub
    End If
   ...
   ' Perform background calculations
    ...
    LabelResult_Y3x.Text = Format(Y3x, "0.00")
    LabelResult_uhat_FOVS.Text = Format(uhat, "0.000")
    LabelResult_FOVTransitionEffort.Text = Format(FOVTransitionEffort, "0.000")
    LabelResult_xhat.Text = Format(xhat, "0")
    LabelResult_CollectionEffort_FOVS.Text = Format(eF, "0")
    ...
    If IsNumeric(LabelResult_xhat.Value) Then
        ...
        LabelResult_xhat.BackColor = RGB(255, 255, 255)
    Else
        LabelResult_LabelResult_xhat.BackColor = RGB(224, 224, 224)
    End If
...
'    CommandButton_SaveVariables_FOVS.Enabled = True
    ...
    If N > xhat And Not MethodSwitchIgnored Then
        ...
        Else
            ...
            CommandButton_FocusMarkers.Visible = True
    ...
    Private Sub UserForm_QueryClose(Cancel As Integer, CloseMode As Integer) ' Once UserForm is closed, close all open windows and clear variables.
        ...
        If CloseMode = 0 Then
            ...
            ElseIf Not SavedVariablesFOVSTargetExists And OutputsSaved Then
                ...
                End If
            ElseIf (txt_N_FOVS.Value <> "" Or txt_N3f.Value <> "" Or txt_LevelError.Value <> "") Or (SavedMarkerDetails Or CalibratedFOV) Then
                Cancel = 1 ' Cancel the close operation
